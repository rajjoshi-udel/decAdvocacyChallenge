...
# Add jitter to prevent overlap on x-axis
np.random.seed(42)
...
for shop in shops:
    ...
    # Add jitter to x-axis (shop ID) to prevent overlap
    jitter_absent = np.random.normal(0, 0.1, len(absent))
    jitter_present = np.random.normal(0, 0.1, len(present))
    
    ax.scatter(absent['shopID'] + jitter_absent, absent['carsFixed'], 
               alpha=0.6, s=40, color=colors['absent'], label='Boss Absent' if shop == shops[0] else '')
    ax.scatter(present['shopID'] + jitter_present, present['carsFixed'], 
               alpha=0.7, s=50, color=colors['present'], label='Boss Present' if shop == shops[0] else '', marker='^')

ax.set_xlabel('Shop', fontsize=12, fontweight='bold')
ax.set_ylabel('Cars Fixed Per Day', fontsize=12, fontweight='bold')
ax.set_title('Productivity by Shop', fontsize=14, fontweight='bold', pad=20)
ax.set_xticks(shops)
...
print(f"\n**Key Insight:** By focusing on Shops 4, 2, and 5, you could add approximately \n{int(annual_strategy_a - annual_current):,} more cars per year compared to your current pattern.")
